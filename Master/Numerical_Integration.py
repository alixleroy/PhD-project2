...
def log_likelihood(y,
    ...
    L = np.log(2/(sigma_l * np.sqrt(2 * np.pi))) - (1/(2 * sigma_l ** 2)) * np.linalg.norm(y - A_mat(u)) ** 2

    return L

def integrate_likelihood(y,
                         bins,
                         sigma_l,
                         mesh,
                         tau,
    ...
    for i in range(len(bins)-1):
        ...
        area_trapezium = ((likelihood_a + likelihood_b)/2) * (b - a)

        likelihood_list.append(likelihood_b.copy())
        int_likelihood_list.append(area_trapezium.copy())

        # likelihood_a = likelihood_b
        log_likelihood_a = log_likelihood_b
    
    return int_likelihood_list, likelihood_list
...
if __name__ == "__main__":
    ...
    def main():
        ...
        int_likelihood_list, likelihood_list = integrate_likelihood(y,
                                                                    bins,
                                                                    sigma_l,
                                                                    mesh,
                                                                    tau,
        ...
        return int_likelihood_list, likelihood_list, bins

    int_likelihood_list, likelihood_list, bins = main()

    print('int_likelihood_list')
    print(int_likelihood_list)
