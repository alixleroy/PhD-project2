...
def master_gen_solver(#nx,
  ...
  '''
  Variation of master solver used to generate data.

  Inputs:
    mesh:         FEniCS mesh:        Mesh which, once refined, is used in finite elements solver.
    tau:          Float:              Rate of growth of hot wall boundary condition in double glazing problem.
    alpha:        Float:              True wind speed parameter to be.
    epsilon:      Float:              Coefficient of diffusion in advection-diffustion equation.
    num_steps:    Int:                Number of steps to run solver for (constant time-stepping).
  Outputs:
    u_list:       List:               List of node values of numerical solution to double glazing problem at each time step.
    t_list:       List:               List of times at which numerical solution is evaluated.
  '''

  # Refine mesh for more accurate approximation.
  mesh = refine(mesh)
  mesh = refine(mesh)
...
if __name__ == '__main__':
  ...
  # Choose resolutions of meshes to interpolate (and project) data onto.
  index = [64, 32]#[2,4,8,16,32,64,128]

  # Interpolate (and project) high resolution data onto coarse meshes.
  for i in range(len(index)):
    ...
    # with open(f'Master/Data_eps100_num150000_tau10_2/master_data_{index[i]}_project_2', 'wb') as file:
    #   pickle.dump(u_coarse_project_list, file, protocol=4)
    with open(f'Master/Data_eps100_num150000_tau10_2/master_data_{index[i]}_interpolate_2', 'wb') as file:
      pickle.dump(u_coarse_interpolate_list, file, protocol=4)
